...
var wechat = function (option) {
    if (typeof option == 'object'){
        ...
        this.debug = (true === option.debug) ? true : false;
    }
    // this._const = config;
};

//设置公众号配置
wechat.prototype.setOptions = function(option){
    if (typeof option != 'object') return false;
    if ( option.appid )          this.appid = option.appid ;
    if ( option.appsecret )      this.appsecret = option.appsecret;
    if ( option.token )          this.token = option.token;
    if ( option.encodingAesKey ) this.encodingAesKey = option.encodingAesKey;
    this.debug = (true === option.debug) ? true : false;
};

//获取公众号配置信息
wechat.prototype.getOptions = function (key) {
    var opt = {
        appid : this.appid,
